...
for y,file in enumerate(files):
    ...
    #f1.plot(b[:-1]/scale,h,'.',color=HSVtoRGB((y+1)/len(P)*0.5,1,1))
    #f1.plot(b[:-1],h,'.',color=(0,0,0))
...
f3 = F3.add_subplot(111)
#f3.plot(P,np.log(DS),'.',color=(0,0,0))
f3.plot(P,DS,'.',color=(0,0,0))
#f3.plot(Pplt,-np.exp(m*Pplt+c),'-',linewidth=1,color=(0,0,0))
...
F1.tight_layout()
F2.tight_layout()
F1.savefig("FigA"+'.pdf', dpi=300, format='pdf')
F2.savefig("FigB"+'.pdf', dpi=300, format='pdf')
